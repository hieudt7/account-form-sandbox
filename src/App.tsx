...
const rgxValidPass = /^(?=.*[0-9])(?=.*[!@#$%^&*])(?=.*[a-zA-Z])[a-zA-Z0-9!@#$%^&*]{8,}$/;
const SignupSchema = yup
  .object({
    password: yup.string().required('Password is required.').min(8).matches(rgxValidPass, 'Minimum eight characters, at least one letter, one number and one special character.'),
    confirmPassword: yup.string().required('Confirm password is required.').min(8).matches(rgxValidPass, 'Minimum eight characters, at least one letter, one number and one special character.').oneOf([yup.ref('password'), null], 'Passwords must match. Please try again!')
  })
  .required();
            ...
            >
              ...
              <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <FormControl mb={8}>
                  ...
                  />
                  {errors.password && <Text className="invalid-lbl">{errors.password?.message}</Text>}
                </FormControl>
                <FormControl mb={8}>
                  <FormLabel
                  ...
                  />
                  {errors.confirmPassword && <Text className="invalid-lbl">{errors.confirmPassword?.message}</Text>}
                </FormControl>
                <FormControl mb={8}>
                  <Flex>
                    <Button
